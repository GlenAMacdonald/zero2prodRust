use actix_web::http::header::{self, HeaderValue, HeaderMap};
...
impl ResponseError for PublishError {
    ...
    // We are providing a bespoke 'error_response' implementation, thus no need to provide a fn 'status_code'
...
fn basic_authentication(headers: &HeaderMap) -> Result<Credentials, anyhow::Error> {
    let header_value = headers
        ...
        .to_str()
        .context("The 'Authorization' header was not avalid UTF8 string.")?;
    ...
    let username = credentials
        .next()
        .ok_or_else(|| {
            anyhow::anyhow!("A username must be provded in 'Basic' auth.")
        })?
        .to_string();
